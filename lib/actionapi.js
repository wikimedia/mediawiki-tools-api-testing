...
/**
 * @file Implements a client for testing action API modules.
 * @module actionapi
...
function failOnError(response, msg, from) {
	from = from || 400;
	...
	if (response.status >= from) {
		...
		assert.fail(`Status code ${ response.status }`,
			`A status code below "${ from }`,
			`${ msg }\n\tUnexpected status ${ response.status }: ${ txt }`);
	}
}
...
class Client {
	 ...
	 * cookie jar.
	 *
	 * Pass in an existing Client instance to share a login session.
	...
	async request(params, post = false, endpoint = 'api.php') {
		...
		endpoint = utils.normalizePath(endpoint);
	...
	async actionWithErrors(actionName, params, post = false, allowErrors = []) {
		...
		failOnError(response, 'The action API should never return an HTTP error status');
		assert.exists(response.body, 'The action API should return a valid JSON payload');
		...
		if (!errors.every(({ code }) => allowErrors.includes(code))) {
			const errorCodes = errors.map(({ code }) => code);
			assert.fail(`User "${ this.username }": Action "${ actionName }" returned error code(s) "${ errorCodes.join(', ') }"!\n${ JSON.stringify(errors) }`);
		}
	...
	async upload(params, file, endpoint = 'api.php') {
		...
		failOnError(res, 'The action API should never return an HTTP error status');
		assert.exists(res.body, 'The action API should return a valid JSON payload');
