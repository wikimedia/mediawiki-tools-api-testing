...
/**
 * @file Provides basic wiki-wide functionality.
 * @module wiki
...
/**
 * Returns a supertest agent for making calls to MediaWiki.
 * If the config has a factory callback set, it will be used to create
 * the superagent instance. Otherwise, this will create a superagent
 * that points to the MediaWiki instance located at the base_uri
 * defined in the config.
 *
 * @return {Object} A supertest agent
 *
 * @namespace api-testing
 */
const agent = () => {
	if (config.factory) {
		...
		// NOTE: we use normalizePath() on the paths to request in REST.js and actionapi.js,
		// which ensures a leading slash. Thus, to avoid a double slash in the request,
		// any trailing slashes must be stripped from base_uri.
		return supertest.agent(
			utils.trimTrailingSlash(config.base_uri)
		);
	}
};
...
const runJobs = async (n = 1) => {
	...
	const sig = (dataIn) => {
		const data = {};
		const keys = Object.keys(dataIn).sort();
	...
	const response = await srv.post('/index.php').type('form').send(params);
...
const getSecretKey = () => config.secret_key || null;
